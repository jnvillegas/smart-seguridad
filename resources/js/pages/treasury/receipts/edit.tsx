...
import type { Client, Receipt, ReceiptItem } from '@/types/treasury';
...
export default function EditReceipt({ receipt, clients }: Props) {
  const { addNotification } = useNotification();
  const [items, setItems] = useState<ReceiptItem[]>(receipt.items || []);

  const { data, setData, put, processing, errors } = useForm({
    client_id: receipt.client_id.toString(),
    fecha_recibo: receipt.fecha_recibo,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    put(`/treasury/receipts/${receipt.id}`, {
      onSuccess: () => {
        addNotification({
          type: 'success',
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Editar ${receipt.numero_recibo}`} />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button type="submit" disabled={processing}>
              {processing ? 'Guardando...' : 'Actualizar Recibo'}
            </Button>
          </div>
        </form>
      </div>
